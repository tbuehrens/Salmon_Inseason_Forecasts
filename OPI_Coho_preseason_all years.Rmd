...
results_best$pct_errpr %>% mean(na.rm=T)

results_best$error %>% mean(na.rm=T)

results_old$forecasts %<>%  mutate(er=predicted_abundance-abundance,
                                   pce=er/abundance)

results_old$forecasts$er%>% mean(na.rm=T)

results_old$forecasts$pce%>% mean(na.rm=T)
...
Figure3.1<-ggplot((results_old$forecasts%>%mutate(model_name="Current_OPI") %>% bind_rows(results_best ) %>% mutate(Model=sub("_"," ",model_name)) %>%  mutate(pct_error=log10(((predicted_abundance-abundance)/abundance)+1)))%>% filter(year<2023),
                  aes(x=year,y=pct_error,fill=Model))+geom_hline(yintercept=0)+geom_col(position=position_dodge(0.5), width=.5)+scale_fill_manual(values=c("darkblue","darkred"))+xlab("")+ylab(expression(paste(log[10], "(% error +1)")))
